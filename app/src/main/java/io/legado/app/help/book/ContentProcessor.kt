...
class ContentProcessor private constructor(
    ...
    fun getContent(
        ...
        val rubyList = arrayListOf<List<RubySpan>>()
        mContent.split("\n").forEach { str ->
            val paragraph = str.trim {
                it.code <= 0x20 || it == '　'
            }
            if (paragraph.isNotEmpty()) {
                val text = if (contents.isEmpty() && includeTitle) {
                    paragraph
                } else {
                    "${ReadBookConfig.paragraphIndent}$paragraph"
                }
                RubyText.parse(text).let {
                    contents.add(it.text)
                    rubyList.add(it.spans)
                }
            }
        }
        return BookContent(sameTitleRemoved, contents, rubyList, effectiveReplaceRules)
    }
