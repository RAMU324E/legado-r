...
import io.legado.app.ui.book.read.page.entities.ReadAloudSegment
...
abstract class BaseReadAloudService : BaseService(),
    ...
    internal var speakContentList = emptyList<String>()
    internal var contentSpeakToDisplayMaps = emptyList<IntArray>()
    internal var contentDisplayToSpeakMaps = emptyList<IntArray>()
    ...
    private fun newReadAloud(play: Boolean, pageIndex: Int, startPos: Int) {
        execute(executeContext = IO) {
            ...
            val readAloudSegments = textChapter.getNeedReadAloudSegments(0, readAloudByPage, 0)
            contentList = readAloudSegments.map(ReadAloudSegment::displayText)
            speakContentList = readAloudSegments.map(ReadAloudSegment::speakText)
            contentSpeakToDisplayMaps = readAloudSegments.map(ReadAloudSegment::speakToDisplayMap)
            contentDisplayToSpeakMaps = readAloudSegments.map(ReadAloudSegment::displayToSpeakMap)
            var pos = startPos
    ...
    internal fun getSpeakText(index: Int): String {
        return speakContentList.getOrElse(index) { contentList.getOrElse(index) { "" } }
    }

    internal fun mapSpeakOffset(index: Int, offset: Int): Int {
        val map = contentSpeakToDisplayMaps.getOrNull(index) ?: return offset
        return map[offset.coerceIn(0, map.lastIndex)]
    }

    internal fun mapDisplayOffset(index: Int, offset: Int): Int {
        val map = contentDisplayToSpeakMaps.getOrNull(index) ?: return offset
        return map[offset.coerceIn(0, map.lastIndex)]
    }
