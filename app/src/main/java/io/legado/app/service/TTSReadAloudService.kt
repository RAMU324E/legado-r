...
class TTSReadAloudService : BaseReadAloudService(), TextToSpeech.OnInitListener {
    ...
    override fun play() {
        ...
        speakJob = execute {
            ...
            for (i in nowSpeak until contentList.size) {
                ensureActive()
                var speakText = getSpeakText(i)
                if (paragraphStartPos > 0 && i == nowSpeak) {
                    val speakStart = mapDisplayOffset(i, paragraphStartPos)
                ...
                if (!isAddedText) {
                    val result = tts.runCatching {
                        speak(speakText, TextToSpeech.QUEUE_FLUSH, null, AppConst.APP_TAG + i)
                    }.getOrElse {
                    ...
                    val result = tts.runCatching {
                        speak(speakText, TextToSpeech.QUEUE_ADD, null, AppConst.APP_TAG + i)
                    }.getOrElse {
                        AppLog.put("tts出错\n${it.localizedMessage}", it, true)
                        TextToSpeech.ERROR
                    }
                    if (result == TextToSpeech.ERROR) {
                        AppLog.put("tts朗读出错:$speakText")
                    }
                }
                isAddedText = true
            }
    ...
    private inner class TTSUtteranceListener : UtteranceProgressListener() {
        ...
        override fun onStart(s: String) {
            ...
            textChapter?.let {
                if (getSpeakText(nowSpeak).matches(AppPattern.notReadAloudRegex)) {
                    nextParagraph()
                }
        ...
        override fun onRangeStart(utteranceId: String?, start: Int, end: Int, frame: Int) {
            ...
            LogUtils.d(TAG, msg)
            textChapter?.let {
                val displayOffset = mapSpeakOffset(nowSpeak, start)
                if (pageIndex + 1 < it.pageSize
                    && readAloudNumber + displayOffset > it.getReadLength(pageIndex + 1)
                ) {
                    pageIndex++
                    ReadBook.moveToNextPage()
