...
data class TextChapter(
    ...
    fun getNeedReadAloudSegments(
        pageIndex: Int,
        pageSplit: Boolean,
        startPos: Int,
        pageEndIndex: Int = pages.lastIndex
    ): List<ReadAloudSegment> {
        if (pages.isEmpty() || pageIndex > lastIndex) {
            return emptyList()
        }
        val endIndex = min(pageEndIndex, pages.lastIndex)
        val segments = arrayListOf<ReadAloudSegment>()
        var lines = arrayListOf<TextLine>()

        fun flushSegment() {
            if (lines.isEmpty()) {
                return
            }
            segments.add(buildReadAloudSegment(lines))
            lines = arrayListOf()
        }

        for (index in pageIndex..endIndex) {
            pages[index].lines
                .filter { it.paragraphNum > 0 }
                .forEach { line ->
                    lines.add(line)
        ...
        return trimReadAloudSegments(segments, startPos)
    }
    ...
    private fun buildReadAloudSegment(lines: List<TextLine>): ReadAloudSegment {
        val displayText = buildString {
            lines.forEach { append(it.text) }
        }
        val speakText = buildString {
            lines.forEach { append(it.readAloudText.ifEmpty { it.text }) }
        }
        val speakToDisplayMap = ArrayList<Int>(speakText.length + 1)
        val displayToSpeakMap = ArrayList<Int>(displayText.length + 1)
        speakToDisplayMap.add(0)
        displayToSpeakMap.add(0)
        var displayOffset = 0
        var speakOffset = 0
        lines.forEach { line ->
            line.readAloudSpeakToDisplayMap
                .drop(1)
                .forEach { speakToDisplayMap.add(displayOffset + it) }
            line.readAloudDisplayToSpeakMap
                .drop(1)
                .forEach { displayToSpeakMap.add(speakOffset + it) }
            displayOffset += line.text.length
            speakOffset += line.readAloudText.ifEmpty { line.text }.length
        }
        return ReadAloudSegment(
            displayText = displayText,
            speakText = speakText,
            speakToDisplayMap = speakToDisplayMap.toIntArray(),
            displayToSpeakMap = displayToSpeakMap.toIntArray()
        )
    }

    private fun trimReadAloudSegments(
        ...
        val result = arrayListOf<ReadAloudSegment>()
        var remain = startPos
        segments.forEach { segment ->
            ...
            val segmentSize = segment.displayText.length + 1
            if (remain >= segmentSize) {
                remain -= segmentSize
                return@forEach
            }
            result.add(segment.trimDisplayStart(remain))
            remain = 0
        }
        return result
    }
