...
import io.legado.app.help.book.RubySpan
...
import io.legado.app.ui.book.read.page.entities.column.RubyColumn
...
class TextChapterLayout(
    ...
    private data class ReadAloudLineData(
        val text: String,
    ...
    private suspend fun getTextChapter(
        ...
        val rubyList = bookContent.rubyList
        ...
        contents.forEachIndexed { index, content ->
            ...
            val paragraphRuby = rubyList.getOrNull(index).orEmpty()
            if (isTextImageStyle) {
                //图片样式为文字嵌入类型
    ...
    private suspend fun setTypeImage(
        ...
        if (size.width > 0 && size.height > 0) {
            ...
            textLine.text = " "
            textLine.readAloudText = " "
            textLine.readAloudSpeakToDisplayMap = intArrayOf(0, 1)
            textLine.readAloudDisplayToSpeakMap = intArrayOf(0, 1)
    ...
    private suspend fun setTypeText(
        ...
        for (lineIndex in 0 until layout.lineCount) {
            ...
            val lineRubyPadding = resolveRubyPadding(rubySpans, lineStart, lineEnd, isTitle)
            val textLine = TextLine(isTitle = isTitle)
            prepareNextPageIfNeed(durY + textHeight + lineRubyPadding)
            val lineText = text.substring(lineStart, lineEnd)
            val (words, widths) = measureTextSplit(lineText, widthsArray, lineStart)
            val desiredWidth = widths.fastSum()
            val readAloudLineData = createReadAloudLineData(text, rubySpans, lineStart, lineEnd)
            textLine.text = lineText
            textLine.readAloudText = readAloudLineData.text
            textLine.readAloudSpeakToDisplayMap = readAloudLineData.speakToDisplayMap
            textLine.readAloudDisplayToSpeakMap = readAloudLineData.displayToSpeakMap
            when {
            ...
            addRubyColumns(textLine, rubySpans, lineStart, lineEnd)
            if (doublePage) {
                textLine.isLeftLine = absStartX < viewWidth / 2
            }
            calcTextLinePosition(textPages, textLine, stringBuilder.length)
            stringBuilder.append(lineText)
            textLine.upTopBottom(durY, textHeight + textLine.rubyTopPadding, fontMetrics)
            val textPage = pendingTextPage
            textPage.addLine(textLine)
            durY += textHeight * lineSpacingExtra + textLine.rubyTopPadding
...
//            }
    ...
    private fun resolveRubyPadding(
        rubySpans: List<RubySpan>,
        lineStart: Int,
        lineEnd: Int,
        isTitle: Boolean
    ): Float {
        if (rubySpans.none { it.start >= lineStart && it.end <= lineEnd }) {
            return 0f
        }
        return ChapterProvider.getRubyPadding(isTitle)
    }

    private fun addRubyColumns(
        textLine: TextLine,
        rubySpans: List<RubySpan>,
        lineStart: Int,
        lineEnd: Int
        ...
        val textColumns = textLine.columns.filterIsInstance<TextColumn>()
        ...
        rubySpans.forEach { span ->
            ...
            val relativeStart = span.start - lineStart
            val relativeEnd = span.end - lineStart
            val startColumn = findTextColumnByOffset(textColumns, relativeStart, false)
                ?: return@forEach
            val endColumn = findTextColumnByOffset(textColumns, relativeEnd, true)
                ?: return@forEach
            if (endColumn.end <= startColumn.start) {
                return@forEach
            }
            textLine.rubyTopPadding = max(
                textLine.rubyTopPadding,
                ChapterProvider.getRubyPadding(textLine.isTitle)
            )
            textLine.addColumn(
                RubyColumn(
                    start = startColumn.start,
                    end = endColumn.end,
                    ruby = span.ruby
                )
            )
    ...
    private fun findTextColumnByOffset(
        columns: List<TextColumn>,
        offset: Int,
        usePreviousWhenBoundary: Boolean
    ): TextColumn? {
        var position = 0
        columns.forEach { column ->
            val next = position + column.charData.length
            if (offset < next) {
                return column
            }
            if (offset == next && usePreviousWhenBoundary) {
                return column
            }
            position = next
        }
        return columns.lastOrNull()
    }

    private fun createReadAloudLineData(
        text: String,
        rubySpans: List<RubySpan>,
        lineStart: Int,
        lineEnd: Int
        ...
        val lineText = text.substring(lineStart, lineEnd)
        val displayLength = lineText.length
        if (displayLength == 0) {
            return ReadAloudLineData("", intArrayOf(0), intArrayOf(0))
        }
        val lineRubySpans = rubySpans
            .filter { it.start >= lineStart && it.end <= lineEnd }
            .sortedBy { it.start }
        if (lineRubySpans.isEmpty()) {
            val speakToDisplayMap = IntArray(displayLength + 1) { it }
            val displayToSpeakMap = IntArray(displayLength + 1) { it }
            return ReadAloudLineData(lineText, speakToDisplayMap, displayToSpeakMap)
        }
        val speakBuilder = StringBuilder(lineText.length)
        val speakToDisplay = arrayListOf(0)
        val displayToSpeak = IntArray(displayLength + 1)
        var speakPos = 0
        var displayPos = 0
        var cursor = lineStart

        fun appendPlain(endExclusive: Int) {
            if (endExclusive <= cursor) {
                return
            }
            val part = text.substring(cursor, endExclusive)
            part.forEach { ch ->
                speakBuilder.append(ch)
                speakPos++
                displayPos++
                speakToDisplay.add(displayPos)
                displayToSpeak[displayPos] = speakPos
            }
            cursor = endExclusive
        }

        fun appendRuby(span: RubySpan) {
            val baseLength = span.end - span.start
            if (baseLength <= 0 || span.ruby.isBlank()) {
                appendPlain(span.end)
                return
            }
            val speakStart = speakPos
            val displayStart = displayPos
            span.ruby.forEachIndexed { index, ch ->
                speakBuilder.append(ch)
                speakPos++
                val consumedDisplay = ((index + 1) * baseLength + span.ruby.length - 1) / span.ruby.length
                speakToDisplay.add(displayStart + consumedDisplay)
            }
            for (offset in 1..baseLength) {
                val consumedSpeak = ((offset * span.ruby.length) + baseLength - 1) / baseLength
                displayToSpeak[displayStart + offset] = speakStart + consumedSpeak
            }
            displayPos += baseLength
            cursor = span.end
        }

        lineRubySpans.forEach { span ->
            appendPlain(span.start)
            appendRuby(span)
        }
        appendPlain(lineEnd)

        return ReadAloudLineData(
            text = speakBuilder.toString(),
            speakToDisplayMap = speakToDisplay.toIntArray(),
            displayToSpeakMap = displayToSpeak
        )
    }
